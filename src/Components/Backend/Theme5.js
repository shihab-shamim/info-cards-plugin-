...
const Theme5 = ({ attributes, card, index, updateCard }) => {
    ...
    const imgEl = isImg && <>
        {img ? <img src={img} alt={title} /> : <MediaPlaceholder label={__(' Card Image:', 'imgBox')} value={{ url: img }} onChange={val => updateCard(index, 'img', val?.url)} />}
    </>
